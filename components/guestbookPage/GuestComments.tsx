...
export default function GuestComments() {
  ...
  if (error) return <pre className="text-xl text-gray-500">{JSON.stringify(error)}</pre>;
        ...
        {data.guestBooksConnection.edges.map((d) => (
          ...
          >
            <div className="flex items-center gap-x-6">
              <h3 className="text-2xl font-semibold capitalize text-main-orange">{d.node.name}</h3>
              <time className="hidden text-xl text-gray-600 sm:block" dateTime="1785-12-30">
                {formateDate(d.node.createdAt)}
              </time>
            </div>
            <p className="mt-4 text-2xl tracking-wider text-gray-400">{d.node.comment}</p>
          </motion.li>
        ))}
      </motion.ul>
    </InfiniteScroll>
